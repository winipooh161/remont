...
<div class="mb-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 mt-4">
        <h5>Файлы договора</h5>
        <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#uploadContractModal">
            <i class="fas fa-upload me-1"></i>Загрузить договор
        </button>
    </div>

    @if($project->contractFiles->isEmpty())
        <div class="alert alert-info">
            В этом разделе будут отображаться файлы договора. 
            Нажмите на кнопку "Загрузить договор", чтобы добавить скан договора или другие связанные документы.
        </div>
    @else
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="uploadContractModalLabel">Загрузить договор</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="uploadContractForm" method="POST" enctype="multipart/form-data" action="{{ route('partner.project-files.store', $project) }}">
                    @csrf
                    <input type="hidden" name="file_type" value="contract">
                    
                    <div class="mb-3">
                        <label for="contractFile" class="form-label">Выберите файл для загрузки</label>
                        <input class="form-control" type="file" id="contractFile" name="file" required>
                        <div class="form-text">Максимальный размер файла: 10 МБ. Поддерживаемые форматы: PDF, DOC, DOCX</div>
                    </div>
                    
                    <div class="mb-3">
                        <label for="contractDescription" class="form-label">Описание файла (необязательно)</label>
                        <textarea class="form-control" id="contractDescription" name="description" rows="2" placeholder="Например: Основной договор, Дополнительное соглашение №1 и т.д."></textarea>
                    </div>
                </form>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="uploadContractButton">Загрузить</button>
            </div>
        </div>
    </div>
</div>
