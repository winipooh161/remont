<div class="mb-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5>Прочие файлы</h5>
        <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#uploadOtherModal">
    ...
    @if($project->otherFiles->isEmpty())
        <div class="alert alert-info">
            ...
            Нажмите на кнопку "Загрузить файлы", чтобы добавить файлы, которые не относятся к другим категориям.
        </div>
    @else
...
<div class="modal fade" id="uploadOtherModal" tabindex="-1" aria-labelledby="uploadOtherModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="uploadOtherModalLabel">Загрузить прочие файлы</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="uploadOtherForm" method="POST" enctype="multipart/form-data" action="{{ route('partner.project-files.store', $project) }}">
                    @csrf
                    <input type="hidden" name="file_type" value="other">
                    
                    <div class="mb-3">
                        <label for="otherFile" class="form-label">Выберите файл для загрузки</label>
                        <input class="form-control" type="file" id="otherFile" name="file" required>
                        <div class="form-text">Максимальный размер файла: 10 МБ.</div>
                    </div>
                    
                    <div class="mb-3">
                        <label for="otherDescription" class="form-label">Описание файла (необязательно)</label>
                        <textarea class="form-control" id="otherDescription" name="description" rows="2" placeholder="Добавьте краткое описание файла"></textarea>
                    </div>
                </form>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="uploadOtherButton">Загрузить</button>
            </div>
        </div>
    </div>
</div>
